...
// A horizontal SOLID PIN / peg with a teardrop cross-section whose apex points
// DOWN (-Z), so it builds up from a 45° base and self-supports. Extends +Y from
// the origin by `length`. Use in place of a plain `cylinder` for FIXED sideways
// pegs (not rotating bearings — a hinge pin must stay round):
