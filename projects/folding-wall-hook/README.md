...
Ready-to-slice STLs are checked in under [`stl/`](stl/) (`frame.stl`,
`paddle.stl`), each pre-oriented for printing. To regenerate, set `part` to
`"frame"` or `"paddle"` and export.

- Suggested: 0.2 mm layers, 3 walls, ≥ 25% infill.
- PLA or PETG both work; PETG is tougher for a load-bearing paddle.
- DFM (`dfm_check`): the only flags are small internal bridges (the pocket floor)
  and the ~0.4 mm feather edge of the countersinks — both inherent and print
  fine; no real thin walls.
